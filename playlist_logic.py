...
def classify_song(song: Song, profile: Dict[str, object]) -> str:
    ...
    # Fix: was checking `title` for chill keywords, but these are genre names — check `genre` instead
...
def compute_playlist_stats(playlists: PlaylistMap) -> Dict[str, object]:
    ...
    # Fix: was dividing len(hype) by len(hype), making hype_ratio always 1.0 — should divide by total songs
    total = len(all_songs)
    hype_ratio = len(hype) / total if total > 0 else 0.0

    avg_energy = 0.0
    if all_songs:
        # Fix: was summing energy only from `hype` songs but dividing by all songs — now sums all songs
...
def search_songs(
    ...
    for song in songs:
        ...
        # Fix: was checking `value in q` (artist name is substring of query) — reversed; should be `q in value`
...
def random_choice_or_none(songs: List[Song]) -> Optional[Song]:
    ...
    # Fix: random.choice([]) raises IndexError — guard with an empty check to return None as the name promises
